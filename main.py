Отлично, теперь вижу все формы. Вот что используешь:

4 августа 10:00 текст— число + месяц
18 августа 12:00 текст
23.10 текст— точки вместо пробела
11.10.2025— с года
8.12 10:00 текст
1.09 КЗ отгрузки— без времени
понедельник 10:00 текст— день неделя
Каждое 1 число месяца 10:00 текст— мысленное повторение
Завтра 10:00— без текста (просто сигнал)
15 диадок— только час без минут
wan— вообще без времени (разовое на завтра?)

Теперь перепишу main.pyполностью с поддержкой всего этого. Замените популярные файлы на GitHub:
питонimport logging
import re
import os
import pytz

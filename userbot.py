...
STATE = {
    "channels": ["vandroukiru","piratesru","samokatus","cheaptrip","travelradar_ru"],
    "keywords": ["Вьетнам"],
    "active": True,
}
...
async def send_to_me(text):
    await telegram.Bot(BOT_TOKEN).send_message(chat_id=MY_CHAT_ID, text=text)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_channels":
        ...
        await query.edit_message_text(f"Мониторинг: {status}", reply_markup=main_keyboard())
        ...
        await query.edit_message_text("⚙️ Управление мониторингом каналов:", reply_markup=main_keyboard())

    elif data == "add_channel":
        context.user_data["waiting"] = "add_channel"
        await query.edit_message_text(
            "Напиши username канала без @ (например: vandroukiru):",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Отмена", callback_data="menu_channels")]])
        )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Отмена", callback_data="menu_keywords")]])
        )

    elif data == "del_channel_list":
        if not STATE["channels"]:
            await query.edit_message_text("Нет каналов.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Назад", callback_data="menu_channels")]]))
            return
        ...
        await query.edit_message_text("Выбери канал для удаления:", reply_markup=InlineKeyboardMarkup(buttons))

    elif data == "del_keyword_list":
        if not STATE["keywords"]:
            await query.edit_message_text("Нет слов.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Назад", callback_data="menu_keywords")]]))
            return
        ...
        await query.edit_message_text("Выбери слово для удаления:", reply_markup=InlineKeyboardMarkup(buttons))
